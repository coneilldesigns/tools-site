...
export default function FeetToInches() {
  ...
  useEffect(() => {
    const maxLength = Math.max(feet.length, inches.length);
    if (maxLength > 8) {
      setTextSize('4vw');
    } else if (maxLength > 6) {
      setTextSize('5vw');
  ...
  const handleFeetChange = (value: number | '') => {
    ...
    setFeet(value.toString());
    const inchesNum = value * 12;
  ...
  const handleInchesChange = (value: number | '') => {
    ...
    const feetNum = value / 12;
    setFeet(formatNumber(feetNum));
  };
